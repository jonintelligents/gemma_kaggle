...
class GemmaChat:
    """
    A class to encapsulate Gemma API calls with support for chat sessions,
    ...
    def __init__(self, api_key: Optional[str] = None, text_model: str = "gemma-3n-e4b-it", 
        ...
        Args:
            api_key: Gemma API key. If None, will try to get from GEMINI_API_KEY env var
        ...
        if not api_key:
            raise ValueError("Please provide api_key or set the GEMINI_API_KEY environment variable")
    ...
    def _make_api_call(self, contents: Any, has_image: bool = False, max_retries: int = 3) -> Dict[str, Any]:
        ...
        for attempt in range(max_retries):
            try:
                self.logger.info(f"Making Gemini API call - attempt {attempt + 1}/{max_retries} using model: {model}")
    ...
    def call_with_tools(self, prompt: str, tool_manager, system_prompt: Optional[str] = None,
        ...
        # Prepare content
        contents = self._prepare_content(prompt, effective_system_prompt, image_path)
    ...
    def call_simple(self, prompt: str, system_prompt: Optional[str] = None,
        ...
        # Prepare content
        contents = self._prepare_content(prompt, effective_system_prompt, image_path, include_history)
