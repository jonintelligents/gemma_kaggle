...
def setup_gemini_api():
    ...
    return genai.GenerativeModel('gemma-2-2b-it')  # Using Gemma 2 2B model

# --- LLM Interaction with Google Gemini API ---

def call_gemini_llm(user_query: str, chat_history: list, model):
    """
...
"""
        ...
        # Convert chat history to Gemini format and include system prompt
        gemini_messages = []
        
        # Add system prompt as first user message
        gemini_messages.append({"role": "user", "parts": [system_prompt]})
        gemini_messages.append({"role": "model", "parts": ["I understand. I'm ready to help you with contact management using the available tools."]})
        ...
        chat = model.start_chat(history=gemini_messages[:-1])  # Exclude the current message
...
def main():
    ...
    print("Using Gemma 2 2B model via Google AI API.")
    ...
    while True:
        ...
        print(f"LLM (Gemma 2 2B via Google AI) processing: '{user_input}'")

        # Step 1: Get the Gemini LLM response (could be text or a tool call JSON)
        llm_response = call_gemini_llm(user_input, chat_history, model)
        ...
        if llm_response["type"] == "tool_code":
            ...
            try:
                ...
                final_llm_response = call_gemini_llm(tool_output_message, chat_history, model)

                if final_llm_response["type"] == "text":
                    print(f"LLM (Gemma 2 2B) final response based on tool output:")
                ...
                print(f"LLM (Gemma 2 2B) response: {error_message}")
            ...
            print(f"LLM (Gemma 2 2B) response: {llm_response['content']}")
            chat_history.append({"role": "assistant", "content": llm_response["content"]})
        else:
            print("LLM (Gemma 2 2B) returned an unexpected response type.")
